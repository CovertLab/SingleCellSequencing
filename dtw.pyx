...
cpdef dtw(
	...
	dist : float
		...
		includes any warping penalty, and is normalized by dividing by the
		natural logarithm of lengths of the sequences, i.e.

		dist = unnormed_dist / log(seq1.size) / log(seq2.size)
	...
	cdef float dist = np.sqrt(warp_penalty*costs[i, j]) / np.log(size1) / np.log(size2)
	# cdef float dist = np.sqrt(costs[i, j])
	# cdef float dist = costs[size1-1, size2-1]
