...
for time in times:
		...
		for j in xrange(number_of_cells):
			if date == "12072015":
				cell = smFISH_stim_cell(cell_id = ids[j], stimulus_condition = conds, NC_ratio = ratios[j,:], norm_med = normalized[j,:], condition = conditions[j], 
					counts = counts[j], medint = medint[j], cyto_fluo = cyto_fluo[j], summed = summed[j], position = positions[j], 
					smFISH_dataframe = data_0, cluster_dynamics_avg = cluster_dynamics_avg)

			if date == "12162015":
				cell = smFISH_stim_cell(cell_id = ids[j], stimulus_condition = conds, NC_ratio = ratios[j,:], norm_med = normalized[j,:], condition = conditions[j],
					counts = counts[j], medint = medint[j], cyto_fluo = cyto_fluo[j], summed = summed[j], position = positions[j], 
					smFISH_dataframe = data_1, cluster_dynamics_avg = cluster_dynamics_avg)
	...
	file_name_save = os.path.join(direc, "good_cells_sTNFR_" + time + "min.pkl")
...
# ax_heatmap_1.xaxis.set_ticklabels(["","0","","","","","","300"])
...
ax_heatmap_3.set_title('sTNFR treated cells - ' + str(number_of_cells) + ' cells', y = 1.05)
ax_heatmap_1.set_xlabel('Time (minutes)')


plt.savefig("plots/sTNFR_dynamics_c1_clustering_300min.pdf")
