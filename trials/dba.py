...
def align_to(fixed, to_align):
	minsize = len(fixed)
	distance, alignment1, alignment2 = dtw(fixed.astype('float64'), to_align.astype('float64'))
	aligned = np.bincount(alignment1, weights = to_align[alignment2], minlength = minsize) / np.bincount(alignment1, minlength = minsize)
	aligned[np.isnan(aligned)] = 0
