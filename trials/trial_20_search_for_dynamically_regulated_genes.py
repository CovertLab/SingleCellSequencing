...
for time_point in times_to_analyze:
	...
	plt.clf()
	fig, axes = plt.subplots(len(inflammatory_genes)/2,2, figsize = (20,20))
	counter = 0
	...
	# 	r("""joint_posterior = scde.posteriors(models = o.ifm[list_of_cells,], counts_data_int, o.prior, n.cores = 4)""")
	# 	jp = extract_dataframe_from_R("joint_posterior")
	# 	file_name_save = os.path.join("/scratch/PI/mcovert/dvanva/sequencing/", str(time_point) + "min_cluster_" + str(cluster) + ".pkl")
	# 	jp.to_pickle(file_name_save)

	if time_point == 300:
		file_name_save = os.path.join("/scratch/PI/mcovert/dvanva/sequencing/", str(time_point) + "min_cluster_" + str(1) + ".pkl")
		jp_1  = pd.read_pickle(file_name_save)

		file_name_save = os.path.join("/scratch/PI/mcovert/dvanva/sequencing/", str(time_point) + "min_cluster_" + str(2) + ".pkl")
		jp_2  = pd.read_pickle(file_name_save)

		file_name_save = os.path.join("/scratch/PI/mcovert/dvanva/sequencing/", str(time_point) + "min_cluster_" + str(3) + ".pkl")
		jp_3  = pd.read_pickle(file_name_save)

		genes = jp_1.index
		list_of_genes = []

		for gene in genes:
			cluster_1_jp = jp_1.loc[gene,:]
			cluster_2_jp = jp_2.loc[gene,:]
			cluster_3_jp = jp_3.loc[gene,:]

			if cluster_1_jp.idxmax(axis = 1) > cluster_2_jp.idxmax(axis = 1) and cluster_1_jp.idxmax(axis = 1) > cluster_3_jp.idxmax(axis = 1):
				list_of_genes += [gene]
	...
	# plt.savefig("plots/" + str(time_point) + "min/inflammatory/" + file_name)
