...
Z_genes = sch.dendrogram(Y_genes, color_threshold = 0.1*max(Y_genes[:,2]))
ind_genes = sch.fcluster(Y_genes, 0.1*max(Y_genes[:,2]),'distance')
ax2.set_xticks([])
ax2.set_yticks([])
time_diff = str(round(time.time()-start_time,1))
...
filename = 'plots/trial_8_dual_clustering_150min_foldchange.pdf'
...
clusters_to_plot = [1,2,4,5,6,7,8,9,10]
...
filename = 'plots/trial_8_dual_clustering_150min_clusters_raw_vszero_reduced.pdf'
